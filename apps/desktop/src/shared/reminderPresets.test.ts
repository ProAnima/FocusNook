...
describe("formatReminderTime", () => {
  it("labels a time later today as 'Сегодня'", () => {
    ...
    expect(formatReminderTime(inTwoHours)).toMatch(/^Сегодня, \d{2}:\d{2}$/);
  });

  it("labels a time tomorrow as 'Завтра'", () => {
    const tomorrow = new Date(NOON);
    tomorrow.setDate(tomorrow.getDate() + 1);
    expect(formatReminderTime(tomorrow.toISOString())).toMatch(/^Завтра, \d{2}:\d{2}$/);
  });

  it("shows the date for anything further out", () => {
    const nextWeek = new Date(NOON);
    nextWeek.setDate(nextWeek.getDate() + 7);
    const result = formatReminderTime(nextWeek.toISOString());
